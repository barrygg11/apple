...
class updateDataCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'update:instances {project} {zone}';
    ...
    public function handle()
    {
        ...
        foreach ($newArray as $key => $respon) {
            ...
            Model::table('MachineList')
            ->where(['project' => $project])
            ->where(['name' => $respon['NAME']])
            ->update(['network_ip' => $respon['NETWORK_IP'],'nat_ip' => $respon['NAT_IP'],'machine_type' => $respon['MACHINE_TYPE'],'status' => $respon['STATUS'],'updated_at' => date('Y-m-d H:i:s')]);
        }
    }
}
